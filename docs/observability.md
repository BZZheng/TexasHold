...
浏览器只上报严格白名单内的诊断字段。服务端不会记录请求体、聊天正文、密码、Cookie、Bearer Token、行动令牌、完整手牌、牌堆、技能三选一候选或技能私密判定结果。
...
`deploy/backup/archive-pull-backup.sh` 会发送固定能力标记 `texas-holdem-backup-v3`；受限导出器只有收到该标记才启用 v3 日志成员并推进日志归档游标。旧版拉取器仍获得不含日志的 v2 业务快照，不能制造“日志已归档”的假健康状态。上线时应先更新归档拉取脚本，再更新应用主机导出器。

v3 协议会附带自上次日志拉取以来（含默认 5 分钟重试窗口）的已轮转日志段。归档端会：
